...
class AccFmuController(BasicControl):
    def __init__(self, actor, args=None):
        ...
        fmu_path = self._resolve_path(
            args.get("fmu"),
            current_dir / controller_cfg.get("fmu_file", "ACC_controller.fmu"),
        )

        parameter_args = dict(config.get("parameters", {}))
        parameter_args.update(args)

        self.parameters = ACCParameters.from_args(parameter_args)
